...
indir="/21dayscratch/scr/w/j/wjzhong/downloads/hic/odc_output" #directory containing input files (e.g. *.pairs files)
suffix="_indexed_contacts.txt.gz" 			#all input files should have the same suffix. it can be an empty string "", or ".txt"
outdir="/21dayscratch/scr/w/j/wjzhong/downloads/hic/odc_output"						#directory where output files will be stored
...
steps="interaction postprocess" 			#steps to run the pipeline. Recommended (1) "bin rwr" at first, (2) then  "hic interaction postprocess"
prefix="odc_100"
