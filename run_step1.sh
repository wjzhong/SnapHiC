...
if [[ "$parallelism" == "parallel" ]]; then
	mpirun -np $number_of_processors python $snapHiC_G_dir/snap.py -i $indir -s $suffix -o $outdir -c $chrs -p $pos -l $chrlen -g $genome --filter-file $filter_file --tss-file $tss_file --promoter-file $promoter_file --enhancer-file $enhancer_file --steps $steps --prefix $prefix --parallel
elif [[ "$parallelism" == "threaded" ]]; then
	python $snapHiC_G_dir/snap.py -i $indir -s $suffix -o $outdir -c $chrs -p $pos -l $chrlen -g $genome --filter-file $filter_file --tss-file $tss_file --promoter-file $promoter_file --enhancer-file $enhancer_file --steps $steps  --prefix $prefix --threaded -n $number_of_processors
else
	python $snapHiC_G_dir/snap.py -i $indir -s $suffix -o $outdir -c $chrs -p $pos -l $chrlen -g $genome --filter-file $filter_file --tss-file $tss_file --promoter-file $promoter_file --enhancer-file $enhancer_file --steps $steps --prefix $prefix
fi
